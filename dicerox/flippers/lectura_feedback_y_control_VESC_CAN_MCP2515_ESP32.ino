...
void convert_data(uint32_t id, uint8_t *data) {
    ...
    if (vesc_id == 0 || vesc_id > 50) return; //Salir en caso de IDs incorrectos
    ...
    case 9:{ // Status 1
        ...
        Serial.printf("ID:%d ERPM:%ld Curr:%.2f Duty:%.3f\n", vesc_id, erpm, current, duty);
        break;
    }
    ...
    case 14:{ // Status 2
        ...
        float amp_hours = ah * 0.0001;
        float amp_hours_charged = ah_ch * 0.0001;

        Serial.printf("ID:%d Ah:%.3f Ah_ch:%.3f\n", vesc_id, amp_hours, amp_hours_charged);
        break;
    }
    ...
    case 15:{ // Status 3
        ...
        float watt_hours = wh * 0.0001;
        float watt_hours_charged = wh_ch * 0.0001;

        Serial.printf("ID:%d Wh:%.3f Wh_ch:%.3f\n", vesc_id, watt_hours, watt_hours_charged);
        break;
    }
    ...
    case 16:{ // Status 4
        ...
        Serial.printf("ID:%d Tf:%.1f Tm:%.1f Iin:%.2f Pos:%.2f\n",
    ...
    case 27:{ // Status 5
        ...
        Serial.printf("ID:%d Tacho:%ld Vin:%.2f\n", vesc_id, tacho, vin);
        break;
    }
